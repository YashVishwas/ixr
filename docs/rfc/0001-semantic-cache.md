...
**Auto-routing scoring against unconfigured providers — fixed, a distinct bug from the above.** `model:"auto"` scored against the *full* routing catalog regardless of which providers a deployment actually configures — `routing.Filter` only ever checked cost cap and circuit-breaker state, never provider availability, and `pkg/ixr/ixr.go` built `scoring.NewEngine` from `routing.Catalog()` with zero reference to the provider `registry` built moments earlier in the same function. A catalog entry with no configured provider could still win the score, and `chat.go` has no recovery for that case: the fallback chain it builds only helps when a provider *resolved* but then failed at runtime (timeout, rate limit, 5xx) — one that never resolved at all hits `h.router(req.Model)`'s error branch and 400s before `Execute` (the only thing that walks a fallback chain) is ever called. Net effect: any deployment configuring fewer providers than the full 8-entry catalog assumes — which is most real deployments — had `"auto"` routinely fail outright instead of falling back to something it could actually serve. Confirmed live: wayfinder's public Playground demo configures only Cerebras, Groq, and Mistral; none of the 8 catalog entries were servable by any of them, so the demo built its own routing heuristic directly into the Playground BFF rather than forwarding `"auto"` to ixr at all.

Fixed with `availableCatalog` (`pkg/ixr/ixr.go`), which filters `routing.Catalog()` down to entries whose provider resolves via `buildRouter` — the same prefix-dispatch resolution `h.router` itself uses, so provider-availability checking can't drift out of sync with actual routing behavior — before that filtered catalog is handed to `scoring.NewEngine`. Also added `gpt-oss-120b` (Cerebras), `llama-3.1-8b-instant` (Groq), and `mistral-small-latest` (Mistral) as real `ModelCard` entries (pricing verified against each provider's own pricing page, matching `wayfinder/playground/internal/catalog.go`'s sourcing; capability priors estimated at the same honest epistemic status as the rest of the catalog — small/cheap models score low enough to lose to a frontier model on a hard reasoning prompt when one's actually configured) — without these, filtering to "configured providers" would have left the demo's registry with zero catalog matches at all. Live-verified end-to-end against a Cerebras+Groq+Mistral-only deployment (the exact wayfinder shape): `"auto"` now picks `llama-3.1-8b-instant` as primary, and — new behavior, previously impossible — correctly falls back to `gpt-oss-120b` when the primary provider call fails, since the fallback chain itself is now built only from configured-provider candidates.
