...
**Status: implemented, all adapters.** OpenAI, openaicompat (and therefore every one of its 11 thin wrappers — Cerebras, DeepSeek, GitHub Models, Llama, llamacpp, local, Mistral, Ollama, OpenRouter, SambaNova, Zhipu), Anthropic, Google AI, and Bedrock all translate `m.Parts` now. Live-verified against real Anthropic with a real generated image — correctly identified its color through the full JSON-polymorphism → adapter-translation → live-API path.

Google AI and Bedrock were the last two open: both translation loops (`googleai/translate.go`, `bedrock/adapter.go`) previously only ever read `m.Content`, never `m.Parts` — a caller's image was silently dropped, request going through as text-only rather than erroring (the documented graceful-degradation trade-off, but previously *silent*: no signal to the caller their image was ignored, only an operator-facing `slog.Warn` added as an interim mitigation). Both now build real image content: Google AI emits `inlineData` (base64) or `fileData` (URL) parts, following the `genPart`-as-one-of pattern already established for `functionCall`/`functionResponse`; Bedrock's Claude endpoint uses the identical wire shape as native Anthropic's Messages API (content-block arrays with `{"type":"image","source":{...}}`), so its fix mirrors that adapter's translation directly. Confirmed via `TestToGenWireRequest_ImageContentTranslated`/`TestToGenWireRequest_ImageHTTPURLPassedThrough` and `TestBuildBody_ImageContentTranslated`/`TestBuildBody_ImageHTTPURLPassedThrough`.
...
**Implemented.** `AdmissionMiddleware` (`internal/ingress/admission_middleware.go`) — a bounded, buffered-channel semaphore sized to a configurable max in-flight request count, wrapping the entire chat handler tree (both the regular chat path and the chain path) rather than living inline inside `ChatHandler.ServeHTTP` as originally sketched, so one acquire/release covers both without duplicated logic. Acquisition is non-blocking — fail fast with 503 `system_overloaded` rather than queueing, matching the RFC's existing latency-budget non-negotiable. Distinct from circuit breaking by design: this rejects based on *aggregate* concurrency regardless of which model is requested, where circuit breaking rejects based on a *specific model's* observed failure rate regardless of system load. The two are complementary, not redundant — a fix to one is not a fix to the other, as this profiling pass demonstrated directly. Configured via `IXR_MAX_INFLIGHT`; unset/`<=0` disables it entirely, restoring exactly the pre-feature behavior. Unit-tested (`admission_middleware_test.go`): under-capacity pass-through, disabled-never-rejects (50 concurrent), at-capacity exact-admit-count with 503 on the rest, and slot-release-after-completion.
