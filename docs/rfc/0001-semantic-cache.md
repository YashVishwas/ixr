...
**Cross-instance circuit breaker state — implemented, found during an audit rather than pre-planned.** `internal/domain/circuitbreaker.Registry` was single-instance only: each replica behind a load balancer independently discovers a model's degradation through its own failed requests, so a model tripped on replica A keeps taking traffic on replicas B and C until each one accumulates enough failures on its own — the exact multi-instance gap Gap 6 (retrieval store) and durable-backend work elsewhere already closed for other stateful components. Auditing the codebase for unfinished work found two separate, dead attempts at this that were never actually wired to anything: `internal/adapters/store/cbstate` (a stub whose `Save` always returned `errNotConnected`) and `internal/adapters/store/circuitstate` (misleadingly named `RedisStore` but was just an in-memory map) — neither referenced by `circuitbreaker.Registry`, which had no pluggable-state interface at all. Both deleted and replaced with one real implementation.

`circuitbreaker.StateStore` (`Load`/`Save`, context + error) is optional — `Registry.WithStateStore` attaches one; unset behaves exactly as before. Deliberately scoped to just the tripped/not-tripped `State` enum, not the full sliding-window bookkeeping (failure counts, window start, probe counts) — replicating that precisely across instances is a much harder coordination problem for marginal benefit, and both dead stubs had already independently converged on state-only as the right scope before either was abandoned. Two places touch the store, both designed around the request-path latency-budget constraint: seeding a freshly created local `Breaker` from the store happens once per model per process lifetime (not per request) with a 200ms timeout, degrading to the default `Closed` state on any error or cache miss; pushing a state *transition* to the store (rare — most calls don't change state) happens in a goroutine so a slow or down Redis can never add latency to a caller's response. A seeded-`Open` breaker's half-open timer restarts from the seeding instance's perspective rather than preserving the original trip time (that timestamp isn't part of what's shared) — imprecise but errs toward extra caution, the safe direction for a circuit breaker to be wrong in. Backend: `internal/adapters/store/cbstate` (Redis), opt-in via `IXR_CBSTATE_REDIS_ADDR`. Unit-tested including a two-independent-client-handles-sharing-one-server test proving the actual multi-instance scenario, not just the interface plumbing.
