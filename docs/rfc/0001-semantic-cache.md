...
**Status: implemented.** OpenAI, openaicompat, and Anthropic (both `data:` URI and URL image sources). Live-verified against real Anthropic with a real generated image — correctly identified its color through the full JSON-polymorphism → adapter-translation → live-API path. Google AI, Bedrock, and Ollama vision support remain open; the interface doesn't require every adapter to support it at once — adapters that don't implement it simply never populate `Parts` on the way out.
