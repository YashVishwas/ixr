#!/usr/bin/env bash
# demo_linux_embed.sh — Go-embed ixr demo for Linux (x86_64 or arm64).
# ixr runs as a goroutine inside a host Go application — same process, same binary.
#
# Usage:  ./demo/demo_linux_embed.sh [branch-name]
#
...
PORT=8091
WORKTREE_BASE="/tmp/ixr-demo-linux-embed"
SERVER_PID=""
EMBED_APP_DIR=""
GOARCH_TARGET="amd64"
...
check_platform() {
  ...
  if [[ "$os" != "Linux" ]]; then
    ...
    echo ""; printf "  Continue anyway? [y/N]: "; read -r ans; [[ "$ans" =~ ^[Yy]$ ]] || exit 0
  fi
  case "$arch" in
    x86_64)  GOARCH_TARGET="amd64" ;;
    aarch64) GOARCH_TARGET="arm64" ;;
    *) echo "  $(yellow "Warning: unknown arch '$arch' — defaulting to amd64.")"; GOARCH_TARGET="amd64" ;;
  esac
}
...
build_ixr() {
  echo ""; echo "  $(bold "Building embed demo (linux/${GOARCH_TARGET})...")"
  write_embed_app
  (cd "$WORKTREE_BASE" && GOOS=linux GOARCH="$GOARCH_TARGET" go build -o "$WORKTREE_BASE/embed-bin" ./cmd/demo-embed/) 2>&1 | sed 's/^/    /'
...
echo ""; echo "  $(bold '╔═════════════════════════════════════════════╗')"; echo "  $(bold '║   ixr  —  interactive demo (Linux · Embed)  ║')"; echo "  $(bold '╚═════════════════════════════════════════════╝')"
