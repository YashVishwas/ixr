#!/usr/bin/env bash
# demo_silicon_docker.sh — Docker-based ixr demo for Apple Silicon (linux/arm64).
#
# Usage:  ./demo/demo_silicon_docker.sh [branch-name]
#
# Requires: git, docker (with linux/arm64 support), python3 (stdlib only)
...
check_platform() {
  ...
  if [[ "$arch" != "arm64" ]]; then
    echo ""
    echo "  $(yellow "Warning: detected architecture '$arch', expected arm64.")"
    echo "  $(yellow "This script targets Apple Silicon. Use demo_intel_docker.sh on Intel Mac.")"
    echo ""
...
check_docker() {
  if ! command -v docker &>/dev/null; then
    echo "  $(red 'Error: docker not found. Install Docker Desktop from https://docker.com')"
    exit 1
  fi
  if ! docker info &>/dev/null; then
    echo "  $(red 'Error: Docker daemon is not running. Start Docker Desktop and try again.')"
    exit 1
  fi
}
...
setup_worktree() {
  ...
  if [[ "$current_branch" == "$BRANCH" ]]; then
    ...
    WORKTREE_BASE="$REPO_ROOT"; return
  fi
...
build_ixr() {
  echo ""
  echo "  $(bold 'Building Docker image (linux/arm64)...')"
  docker build --platform linux/arm64 -t "$IMAGE_TAG" "$WORKTREE_BASE" 2>&1 | sed 's/^/    /'
  echo "  $(green 'Image built.')"
}

# ── config ────────────────────────────────────────────────────────────────────
write_demo_config() {
  # Expand env vars at write time — container reads plain values, no ${VAR} refs needed.
...
start_server() {
  echo ""
  echo "  $(bold "Starting ixr container on port ${PORT}...")"
  docker run -d \
    --name "$CONTAINER_NAME" \
  ...
  until curl -sf "http://localhost:$PORT/v1/chat/completions" \
    ...
    if ! docker inspect "$CONTAINER_NAME" --format '{{.State.Running}}' 2>/dev/null | grep -q true; then
      echo "  $(red 'Container failed to start. Logs:')"
...
echo ""
echo "  $(bold '╔═══════════════════════════════════════════════╗')"
echo "  $(bold '║   ixr  —  interactive demo (Silicon · Docker) ║')"
echo "  $(bold '╚═══════════════════════════════════════════════╝')"
