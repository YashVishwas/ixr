#!/usr/bin/env bash
# demo_silicon_embed.sh — Go-embed ixr demo for Apple Silicon.
# ixr runs as a goroutine inside a host Go application — same process, same binary.
#
# Usage:  ./demo/demo_silicon_embed.sh [branch-name]
#
...
PORT=8089
WORKTREE_BASE="/tmp/ixr-demo-silicon-embed"
SERVER_PID=""
EMBED_APP_DIR=""   # set after worktree is known
...
check_platform() {
  ...
  if [[ "$arch" != "arm64" ]]; then
    echo ""; echo "  $(yellow "Warning: detected '$arch', expected arm64 (Apple Silicon).")"
    echo "  $(yellow "Use demo_intel_embed.sh on Intel Mac.")"
...
cleanup() {
  ...
  # Remove the temporary embed app source and binary
  if [[ -n "$EMBED_APP_DIR" ]] && [[ -d "$EMBED_APP_DIR" ]]; then
    rm -rf "$EMBED_APP_DIR"
  fi
...
setup_worktree() {
  ...
  EMBED_APP_DIR="$WORKTREE_BASE/cmd/demo-embed"
}
...
build_ixr() {
  echo ""; echo "  $(bold 'Building embed demo (ixr inside host app)...')"
  write_embed_app
  (cd "$WORKTREE_BASE" && go build -o "$WORKTREE_BASE/embed-bin" ./cmd/demo-embed/) 2>&1 | sed 's/^/    /'
...
echo ""
echo "  $(bold '╔═══════════════════════════════════════════════╗')"
echo "  $(bold '║   ixr  —  interactive demo (Silicon · Embed)  ║')"
echo "  $(bold '╚═══════════════════════════════════════════════╝')"
echo "  $(cyan 'Embed mode:') ixr runs as a goroutine inside the host app binary."
echo ""
