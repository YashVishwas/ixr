#!/usr/bin/env bash
# demo_linux_docker.sh — Docker-based ixr demo for Linux (x86_64 or arm64).
#
# Usage:  ./demo/demo_linux_docker.sh [branch-name]
#
...
PORT=8087
WORKTREE_BASE="/tmp/ixr-demo-linux-docker"
CONTAINER_NAME="ixr-demo-linux"
IMAGE_TAG="ixr-demo:linux"
DOCKER_PLATFORM="linux/amd64"
LOG_FILE="/tmp/ixr-linux-docker.log"
DOCKER_LOG_PID=""
...
check_platform() {
  ...
  if [[ "$os" != "Linux" ]]; then
    echo ""; echo "  $(yellow "Warning: detected OS '$os', expected Linux.")"
    echo "  $(yellow "Use demo_silicon_docker.sh or demo_intel_docker.sh on macOS.")"
  ...
  case "$arch" in
    ...
    *) echo "  $(yellow "Warning: unknown arch '$arch' — defaulting to linux/amd64.")"; DOCKER_PLATFORM="linux/amd64" ;;
  esac
}

check_docker() {
  if ! command -v docker &>/dev/null; then
    echo "  $(red 'Error: docker not found. Install from https://docs.docker.com/engine/install/')"; exit 1
  fi
  if ! docker info &>/dev/null; then
    echo "  $(red 'Error: Docker daemon is not running. Start it with: sudo systemctl start docker')"; exit 1
  fi
}
...
build_ixr() {
  echo ""; echo "  $(bold "Building Docker image (${DOCKER_PLATFORM})...")"
  docker build --platform "$DOCKER_PLATFORM" -t "$IMAGE_TAG" "$WORKTREE_BASE" 2>&1 | sed 's/^/    /'
...
echo ""
echo "  $(bold '╔══════════════════════════════════════════════╗')"
echo "  $(bold '║   ixr  —  interactive demo (Linux · Docker)  ║')"
echo "  $(bold '╚══════════════════════════════════════════════╝')"
