#!/usr/bin/env bash
# demo_linux.sh — Interactive ixr demo for Linux (x86_64 or arm64).
#
# Usage:  ./demo/demo_linux.sh [branch-name]
#
...
PORT=8083
WORKTREE_BASE="/tmp/ixr-demo-linux"
SERVER_PID=""
GOARCH_TARGET="amd64"
...
check_platform() {
  ...
  if [[ "$os" != "Linux" ]]; then
    echo ""
    echo "  $(yellow "Warning: detected OS '$os', expected Linux.")"
    echo "  $(yellow "This script targets Linux. Use demo_silicon.sh or demo_intel.sh on macOS.")"
    echo ""
  ...
  case "$arch" in
    ...
    *)
      echo "  $(yellow "Warning: unrecognised architecture '$arch' — defaulting to amd64.")"
      GOARCH_TARGET="amd64"
      ;;
  esac
}
...
build_ixr() {
  echo ""
  echo "  $(bold "Building ixr (linux/${GOARCH_TARGET})...")"
  (cd "$WORKTREE_BASE" && GOOS=linux GOARCH="$GOARCH_TARGET" go build -o "$WORKTREE_BASE/ixr-bin" ./cmd/ixr/) 2>&1 | sed 's/^/    /'
...
echo ""
echo "  $(bold '╔══════════════════════════════════════════╗')"
echo "  $(bold '║      ixr  —  interactive demo (Linux)    ║')"
echo "  $(bold '╚══════════════════════════════════════════╝')"
